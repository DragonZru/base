...
BEGIN
    ...
    while counter < table_count
        do
            set @sql = concat('CREATE TABLE IF NOT EXISTS t_example_', counter, ' (
    `id`          bigint UNSIGNED NOT NULL AUTO_INCREMENT,
    `username`    varchar(255)    NOT NULL,
    `password`    varchar(255)    NOT NULL,
    `version`     bigint          NOT NULL DEFAULT 0,
    ...
    PRIMARY KEY (`id`),
    UNIQUE INDEX `U_username`(`username`),
    INDEX n_extras( ( CAST( extras ->> ''$[*].serialNo'' AS CHAR(30)) COLLATE utf8mb4_bin) ),
    FULLTEXT INDEX f_value (value) WITH PARSER ngram);
');
